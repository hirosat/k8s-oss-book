...
namespace付きのタグを生成
```
$ docker tag node-app:v1.0.0 hirosat/node-app:v1.0.0
```
...
```
$ docker push hirosat/node-app:v1.0.0
```

### Dockerfileのベストプラクティス
#### イメージレイヤーの最適化
ポイント1: package*.json を事前にコピーする
```
FROM node:18.9.0
WORKDIR /app
COPY package*.json ./
...
```
$ docker image inspect nginx --format="{{json .Config.Labels}}" | jq
{
...
### pack CLIのインストール
pack CLIのダウンロード<br>
(※. [リリースページ](https://github.com/buildpacks/pack/releases)より、お使いのOSの最新版のURLをコピー)
```
$ wget (pack CLI URL)
```
(上記のコマンドにより、圧縮ファイルがローカルに設置される)
...
コンテナの起動
```
$ docker run -d -p 3000:3000 cnb-app:latest
```
